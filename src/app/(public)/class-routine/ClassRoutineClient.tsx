...
export default function ClassRoutineClient({ 
  heroData,
  cardData
}: { 
  heroData?: any;
  ...
  const routineImageSrc = cardData?.mediaUrl || cardData?.fileUrl || heroData?.fileUrl || heroData?.mediaUrl || "/images/flyer_hsc26_hsc27.jpg";
    ...
    <div className="min-h-screen bg-[#FFF9F2] pt-24 pb-24 relative overflow-hidden">
      ...
      <div className="brand-container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 relative z-10">
        ...
        <InnerPageHero
          ...
          title={
            <>
              <span className="block text-white">{heroData?.title || "Class Routine"}</span>
              <span className="block text-accent mt-1">{heroData?.subtitle || "Session 2026 - 2027"}</span>
            </>
          }
          description={heroData?.description || "Stay on track with Shifat Sir's complete weekly class routine, batch timings, and live lecture schedules."}
          breadcrumbs={[
            { label: "Home", href: "/" },
            { label: "Academic" },
            { label: "Class Routine" }
          ...
          imageAlt="Class Routine Cover"
        />

        {/* Single Massive Routine Image Body */}
          ...
          <div className="flex items-center justify-between pb-4 sm:pb-6 border-b border-gray-100 mb-6">
            <div className="flex items-center space-x-3">
              ...
              <h3 className="font-extrabold text-[#08132E] text-base sm:text-lg">
                Official Class Routine & Weekly Schedule
              </h3>
            </div>
            <div className="flex items-center gap-3">
              ...
              <a
                href={routineImageSrc}
                download="Shifats-Tales-Class-Routine.jpg"
                target="_blank"
          ...
          >
            ...
            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/15 transition-colors flex items-center justify-center pointer-events-none">
              <span className="opacity-0 group-hover:opacity-100 transition-opacity bg-[#08132E]/90 text-white font-bold px-6 py-3 rounded-full text-sm shadow-xl flex items-center gap-2">
                <Maximize2 className="w-4 h-4 text-accent" /> Click to enlarge full routine
              </span>
            </div>
          </div>
        </div>
