...
export default function CoursesClient({ heroData, courseItems = [] }: { heroData?: any, courseItems?: any[] }) {
  ...
  const categories = [
    { id: "all", label: "All Programs", icon: Star },
  ...
  const filteredCourses = courseItems.filter((course) => {
    ...
    return course.metadata?.examType === filter;
  });
        ...
        <motion.div layout className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-8 sm:gap-10">
          <AnimatePresence mode="popLayout">
            {filteredCourses.map((course) => {
              ...
              const bannerImage = course.mediaUrl || meta.fallbackImageUrl || "/images/flyer_admission_science.jpg";
              const target = meta.target || "All";
              const title = course.title;
              const subtitle = course.subtitle;
              const description = course.body;
              const features = meta.features || [];
              const schedule = meta.schedule || "N/A";
              ...
              <div className="p-7 md:p-8 flex flex-col flex-grow relative">
                ...
                <p className="text-[13px] sm:text-sm text-gray-600 leading-relaxed mb-6 font-medium whitespace-pre-line">
                  {description}
                </p>
                ...
                <div className="grid grid-cols-2 gap-5 p-5 rounded-3xl bg-[#08132E] text-white text-left mt-auto mb-6 shadow-md relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    ...
                    <span className="text-xs sm:text-[13px] font-medium leading-snug text-white/95 block pr-2">{schedule}</span>
                  </div>
                  <div className="relative z-10 border-l border-white/10 pl-5">
                    ...
                    <span className="text-xs sm:text-[13px] font-medium leading-snug text-white/95 block">{duration}</span>
                  </div>
                </div>
                ...
                <a
                  href={`https://wa.me/${whatsappNumber}?text=${encodeURIComponent(
                    `Hello Sir, I want to inquire about the batch: ${title}. Please provide details and class timings.`
                  )}`}
                  target="_blank"
