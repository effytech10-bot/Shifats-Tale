...
export function CalendarCardForm({ 
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Academic Calendar image saved successfully!");
      router.refresh();
    } catch (err: any) {
      toast.error(err.message || "Failed to update calendar image.");
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <form onSubmit={handleSave} className="space-y-6 bg-white p-6 sm:p-8 rounded-3xl border border-border/60 shadow-sm">
      <div className="space-y-4">
        <label className="block text-sm font-extrabold text-primary flex items-center gap-2">
          ...
          Full Academic Calendar Image (Card Section)
        </label>
        <p className="text-xs text-muted leading-relaxed font-medium">
          Upload a new high-resolution academic calendar/schedule image or choose from existing uploaded media in the library. This massive flyer will be displayed prominently on the Academic Calendar page.
        </p>
